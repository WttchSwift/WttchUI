...
public struct CustomPicker<T: Hashable & Identifiable, Content: View, Label: View>: View {
    // 要渲染的数据
    let data: [T]
    // 绑定值
    ...
    ///   - content: 项目渲染视图的闭包方法
    ///   - label: 标签
    public init(_ data: [T], selection: Binding<T?>, @ViewBuilder content: @escaping (T?) -> Content,  @ViewBuilder label: () -> Label) {
        self.data = data
    ...
    static var previews: some View {
        VStack {
            GeometryReader { geo in
                CustomPicker(datas, selection: .constant(Data(id: 1)), content: { i in
                    if let i = i {
                        ZStack {
